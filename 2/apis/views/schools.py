...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        credits_context = [
            {
                "id": 6,
                "credit": 1,
                "subject_id_list_that_using_this_credit": [3],
            },
            {
                "id": 7,
                "credit": 2,
            ...
            },
            {
                "id": 9,
                "credit": 3,
                "subject_id_list_that_using_this_credit": [1, 5],
            },
        ]
        ...
        if (
            ...
            return Response(
                data={"error": "wrong data type"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        student_object = Personnel.objects.filter(
        ...
        subject_object = Subjects.objects.filter(
        ...
        credit = filter(
            lambda x: x["subject_id"] == subject_object.id, credits_mapping
        )
        credit = list(credit)[0].get("credit_id")
        StudentSubjectsScore.objects.create(
            student=student_object,
            subjects=subject_object,
            score=score,
            credit=credit,
        )
    ...
    def get(request, *args, **kwargs):
        ...
        for subject in subjects:
            ...
            if subject.score >= 80:
                ...
                grade = "D"
            score_list.append(subject.score)
        ...
        example_context_data = {
            ...
            "grade_point_average": sum(score_list) / len(score_list),
        }
...
class PersonnelDetailsAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        personel_school = Personnel.objects.filter(
        ...
        your_result = []
        for index, personel in enumerate(personel_school, 1):
            your_result.append(
                f"{index}. school: {personel.school_class.school.title}, role: {role_mapping[personel.personnel_type]}, class: {personel.school_class.class_order}, name: {personel.first_name} {personel.last_name}."
            )
    ...
    def get(request, *args, **kwargs):
        ...
        for school in all_schools:
            ...
            for classroom in classes:
                class_detail = {
                    f"class {classroom.class_order}": {
                        f"Teacher: {classroom.personnel_set.filter(personnel_type=0).first().first_name} {classroom.personnel_set.filter(personnel_type=0).first().last_name}": [
                            {
                                "Head of the room": f"{classroom.personnel_set.filter(personnel_type=1).first().first_name} {classroom.personnel_set.filter(personnel_type=1).first().last_name}"
                            }
                        ],
                    }
                }
                students = classroom.personnel_set.filter(personnel_type=2)
                for student in students:
                    class_detail[f"class {classroom.class_order}"][
                        f"Teacher: {classroom.personnel_set.filter(personnel_type=0).first().first_name} {classroom.personnel_set.filter(personnel_type=0).first().last_name}"
                    ].append(
                        {
                ...
                school_detail.update(class_detail)
            your_result.append(school_detail)
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        your_result = []
        school_structure = SchoolStructure.objects.filter(parent=None)
        for structure in school_structure:
            main_structure = SchoolStructure.objects.filter(parent=structure)
            structure_data = {
                "title": structure.title,
                "sub": [],
            }
            for m_structure in main_structure:
                sub_structure = SchoolStructure.objects.filter(
                    parent=m_structure
                )
                m_structure_data = {
                    "title": m_structure.title,
                }
                sub_structure_list = []
                for s_structure in sub_structure:
                    sub_structure_list.append({"title": s_structure.title})
                m_structure_data["sub"] = sub_structure_list
                structure_data["sub"].append(m_structure_data)

            your_result.append(structure_data)
        return Response(your_result, status=status.HTTP_200_OK)
